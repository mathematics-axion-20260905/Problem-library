...
/** A self-contained, JSON-safe package for moving an object between ecosystem apps/origins. */
...
export function serializeScientificObject<TPayload>(
  object: ScientificObject<TPayload>,
  revisions: Array<ScientificObjectRevision<TPayload>> = object.revision ? [object.revision] : [],
): string {
  const { revision: _currentRevision, ...metadata } = object;
  return JSON.stringify({
    transferSchemaVersion: SCIENTIFIC_OBJECT_TRANSFER_SCHEMA_VERSION,
    exportedAt: new Date().toISOString(),
    object: metadata,
    revisions,
  } satisfies ScientificObjectTransferEnvelope<TPayload>);
}

export function deserializeScientificObject<TPayload>(
  serialized: string,
): ScientificObjectTransferEnvelope<TPayload> {
  let parsed: Partial<ScientificObjectTransferEnvelope<TPayload>>;
  try {
    parsed = JSON.parse(serialized) as Partial<ScientificObjectTransferEnvelope<TPayload>>;
  } catch {
    throw new Error("INVALID_SCIENTIFIC_OBJECT_TRANSFER_ENVELOPE");
  }
  if (parsed.transferSchemaVersion !== SCIENTIFIC_OBJECT_TRANSFER_SCHEMA_VERSION || !parsed.object || !Array.isArray(parsed.revisions)) {
    throw new Error("INVALID_SCIENTIFIC_OBJECT_TRANSFER_ENVELOPE");
  }
  if (!parsed.object.id || !parsed.object.projectId || !parsed.object.schemaVersion || !parsed.object.kind || !parsed.object.sourceApp || parsed.object.currentRevision < 1) {
    throw new Error("INVALID_SCIENTIFIC_OBJECT_TRANSFER_OBJECT");
  }
  const revisionNumbers = parsed.revisions.map((revision) => revision.revision);
  if (!parsed.revisions.length || new Set(revisionNumbers).size !== revisionNumbers.length || !parsed.revisions.some((revision) => revision.revision === parsed.object?.currentRevision) || parsed.revisions.some((revision) => revision.objectId !== parsed.object?.id || revision.revision < 1 || !revision.provenance)) {
    throw new Error("INVALID_SCIENTIFIC_OBJECT_TRANSFER_REVISIONS");
  }
